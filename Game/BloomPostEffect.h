...
private:
	...
	void	filterBright(const sf::Texture& input, sf::RenderTexture& output);
	void	blurMultipass(RenderArray& renderTextures);
	void	blur(const sf::Texture& input, sf::RenderTexture& output, sf::Vector2f offsetFactor);
	void	downsample(const sf::Texture& input, sf::RenderTexture& output);
